...
class ShipmentController extends Controller
{
    ...
    public function shipments()
    {
        ...
        foreach ($shipments as $value) {
            $shipments_ids[] = $value->id;
        }

        $items = ShipmentItem::whereIn('shipment_id', $shipments_ids)->get();
        ...
        $sells = ShipmentItemSell::whereIn('shipment_item_id', $items_ids)->get();
    ...
    public function addShipment(Request $request)
    {
        ...
        if (!empty($check_shipments)) {
            //get the year date and get two last number
            $year_date = Date('Y');

            $year_date = substr($year_date, 2);

            $request->number = $request->number . '-' . $year_date;

            //check the  shipment with year value
            $check_shipments = Shipment::where('number', $request->number)->first();

            if (!empty($check_shipments)) {
                return redirect()
                    ->back()
                    ->with('warning', 'عذرا رقم الإرسالية و سنة الإرسالية( ' . $request->number . ' ) مستخدم من قبل');
            }
        }
        ...
        return redirect()
            ->back()
            ->with('success', ' تم اضافة إرسالية بنجاح ' . $dub_items);
    }
    ...
    public function addProductShipment(Request $request)
    {
        ...
        return redirect()
            ->back()
            ->with('success', ' تم اضافة بيان جديد للإرسالية بنجاح ' . $dub_items);
    }
    ...
    {
        $check_number = Shipment::where('number', $request->number)
        ->where('id', '!=', $request->id)
        ->first();

        if (!empty($check_number)) {

            return redirect()->back()->with('warning', 'رقم الإرسالية مستخدم من قبل');
        ...
        return redirect()->back()->with('success', 'تم تعديل الإرسالية بنجاح');
    }
    ...
    public function sellShipment(Request $request)
    {
        ...
        if ($selling->remaining_quantity < 0) {
            return redirect()
                ->back()
                ->with('warning', 'عذرا الكمية المباعة اكثر من الكمية المتبيقة في الشحنة  ( ' . $item->remaining_quantity . ' )');
        }
    ...
    public function UpdateSell(Request $request)
    {
        ...
        $all_selling = ShipmentItemSell::where('shipment_item_id', $item->id)
            ->where('id', '!=', $request->id)
            ->sum('quantity');
        $all_damaged = ShipmentItemSell::where('shipment_item_id', $item->id)
            ->where('id', '!=', $request->id)
            ->sum('damaged');
        ...
        if ($selling->remaining_quantity < 0) {
            return redirect()
                ->back()
                ->with('warning', 'عذرا الكمية المباعة اكثر من المتبقية في الشحنة  ( ' . $item->remaining_quantity . ' )');
        }
